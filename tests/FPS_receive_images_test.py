"""FPS_receive_images_test.py -- receive images, then print FPS stats

A test program to provide FPS statistics as different imagenode algorithms are
being tested. This program receives images and computes and prints FPS
statistics. It requires a FPS_test.yaml file with appropriate options in the
home directory of the receiving computer. There is a similar program that
receives images rather than jpg compressed images.
...
2. Run this program in its own terminal window on the mac:
   python FPS_receive_images_test.py.
